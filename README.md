...
메뉴바 popover, 플로팅 펫, helper 버튼, WidgetKit처럼 실제 UI를 누르기 전에는 read-only QA 게이트를 먼저 실행한다. 기본 모드는 설치된 앱이 최신 `dist/MacDog.app`와 다르면 실패하고, WidgetKit fixture writer는 self-test 모드로만 확인해 live cache를 건드리지 않는다.
