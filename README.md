...
- 메뉴바 앱: 앱 소유 cache 기반 사용량 표시, 사용량에 따라 움직이는 강아지 러너, 우측 세로 탭 popover, 우클릭 메뉴
- Codex 탭: 5시간/주간 사용률, reset까지 남은 시간, plan, 갱신 시각
- 활성 자원 탭: CPU, 메모리, 저장 용량, 네트워크 상태와 1초 단위 로컬 갱신
- 잠들지 않기 탭: 끔/시간 제어/상태 기준, IOKit assertion, 덮개 닫힘 보호
- 배터리 탭: 배터리/전원 상태와 macOS 충전 한도 읽기/적용
- 데스크톱 플로팅 펫: 화면 경계 안 이동, 드래그 위치 저장, 좌클릭 popover, 우클릭 메뉴, CPU/배터리 상태 반응
...
`verify_privileged_helper_xpc.sh`는 권한 도우미 연결 확인용 스크립트다. 새로 빌드된 `dist/MacDog.app`이 있으면 그 실행 파일을 우선 사용하고, 없으면 설치된 앱을 사용한다.
helper 실제 설치 전에는 아래 preflight로 helper bundle, dry-run, 현재 설치 상태, 권한 도우미 연결 진단 경로를 먼저 확인한다.

```sh
./script/verify_privileged_helper_preflight.sh --build
```

메뉴바 popover, 플로팅 펫, helper 버튼, WidgetKit처럼 실제 UI를 누르기 전에는 read-only QA 게이트를 먼저 실행한다. 기본 모드는 설치된 앱이 최신 `dist/MacDog.app`와 다르면 실패하고, 앱 UI process가 Codex app-server나 Group Containers fallback을 직접 열지 않는지도 확인한다. WidgetKit fixture writer는 self-test 모드로만 확인해 live cache를 건드리지 않는다.

```sh
./script/verify_manual_ui_prerequisites.sh
```

WidgetKit 수동 검수에서 stale/error 상태를 재현해야 하면, 먼저 self-test로 live cache를 건드리지 않는지 확인한 뒤 shared cache fixture를 쓴다.
...
cache writer는 기존 Application Support cache와 WidgetKit shared cache에 함께 기록한다. 메뉴바 앱은 앱 소유 Application Support cache를 읽고, Widget extension은 shared cache를 읽는다. 메뉴바 앱 UI process는 Codex app-server나 Group Containers fallback을 직접 열지 않는다.
...
권한 도우미 설치/삭제 계약, 설치/삭제 스크립트, helper 우선 제어 코드, 잠들지 않기 탭의 helper 상태별 안내와 앱 내부 설치/제거 버튼은 준비되어 있다. 앱 내부 버튼은 앱 번들에 포함된 helper를 사용하고 관리자 승인 후 `/Library/PrivilegedHelperTools`와 `/Library/LaunchDaemons`만 변경한다. 2026-05-28 기준 helper-only 실제 설치, 실제 삭제/재설치, 권한 도우미 연결을 통한 조회/변경/복구 검증, 최신 설치본 UI에서 `시간 제어`/`끔` 클릭 검증, 짧은 덮개 닫힘 실기 검증, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 재검증까지 완료했다. 대조군으로 `SleepDisabled=0`에서 덮개를 닫으면 즉시 잠금이 걸리고 검정 화면/비밀번호 화면으로 이어지는 것도 확인했다. 2026-05-28 23:50 KST부터 전원 연결 기준 슬립 방지와 `SleepDisabled=1` 상태로 장시간 에이징을 시작했고, 2026-05-29 사용자 실사용 확인에서 덮개 닫힘 상태가 슬립/락으로 떨어지지 않았다. 2026-05-29 앱 내부 helper 제거/설치 버튼 실제 클릭 흐름을 검수했고, 최신 설치본에서는 helper 설치/제거 전에 MacDog가 변경할 시스템 위치를 먼저 안내하는 것을 확인했다.

주의: UI 검수 전에는 `./script/verify_install_state.sh --expect-current-dist`로 `/Users/dhseo/Applications/MacDog.app`이 최신 `dist/MacDog.app`와 같은 빌드인지, 실행 중인 MacDog가 설치된 앱 binary인지 확인한다. 이전 설치본이 실행 중이면 helper 연동 이전 경로로 비밀번호 프롬프트가 발생할 수 있다.
...
Shortcuts 보조 경로는 `script/verify_shortcuts_charge_limit.sh`로 read-only 상태와 Charge Limit 후보 단축어 이름만 확인한다. 현재 개발 Mac에서는 Shortcuts helper application 통신 실패가 발생할 수 있어, 표준 체크는 available/unavailable 상태를 기록만 하고 native Charge Limit 경로를 기본 구현으로 유지한다.
