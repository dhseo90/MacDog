...
- 메뉴바 popover: 5시간/주간 사용량, plan, credits, 마지막 업데이트 표시
...
- CLI MVP, shared cache, macOS 메뉴바 앱, Codex Pup 러너, 데스크톱 플로팅 펫 MVP 구현 완료
- `MacDog`는 SwiftPM executable로 빌드한다.
- WidgetKit 지원 코드는 library로 존재하지만, 설치 스크립트는 아직 `.appex` 위젯 번들을 배포하지 않는다.
- 데스크톱/알림 센터 위젯 검수는 Xcode Widget Extension target 연결 이후 별도 단계에서 진행한다.
