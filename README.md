...
우측 탭 버튼 이미지는 Codex Pup 캐릭터 프로필의 popover tab artwork로 연결되어 있다. 같은 프로필이 데스크톱 펫 sprite와 메뉴바 러너 frame도 함께 소유하므로, 후속 캐릭터를 추가할 때는 버튼/데스크톱/메뉴바 asset을 한 묶음으로 바꾼다.
