...
CLI MVP와 shared cache 구현 단계다. 로컬 Codex app-server 사용량 조회, `codex-usage status --json` live smoke, 임시 cache write smoke를 검증했고, 다음 단계는 macOS menu bar 앱 구현이다.

macOS menu bar 앱 MVP는 SwiftPM `CodexUsageMonitor` executable로 빌드된다. 직접 실행 검수는 별도 수동 확인 단계에서 진행한다.
메뉴바 앱은 runner/spark/pulse 테마, max/5h/weekly 기준 선택, reduced motion 옵션을 포함한다.
WidgetKit 지원 코드는 SwiftPM `CodexUsageWidget` library로 빌드된다. 실제 위젯 추가/실행 검수는 별도 수동 확인 단계에서 진행한다.
