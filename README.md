...
메뉴바 앱은 Pup/Spark Pup/Pulse Pup 테마, max/5h/weekly 기준 선택, reduced motion 옵션을 포함한다.
Runner speed의 기본 기준은 주간 사용량이며, 사용자가 직접 바꾼 선택값은 이후 실행에서도 유지된다.
WidgetKit 지원 코드는 SwiftPM `CodexUsageWidget` library로 빌드된다. 현재 설치 스크립트는 `.appex` 위젯 번들을 만들거나 설치하지 않는다. 실제 데스크톱/알림 센터 위젯 추가와 실행 검수는 Xcode extension 타깃을 붙인 뒤 별도 수동 확인 단계에서 진행한다.
