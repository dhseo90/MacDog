...
require_contains "$output" "MacDog.app (includes bundled codex-usage, macdog-claude-statusline bridge, and macdog-grok-usage writer)"
...
if [[ -d "$APP_BUNDLE" && -n "${MACDOG_RELEASE_VERSION:-}" ]]; then
  ...
  [[ -x "$stage/MacDog.app/Contents/MacOS/macdog-grok-usage" ]] || die "bundled Grok usage writer missing or not executable"
