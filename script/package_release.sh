...
Privileged helper: not installed by this release command; helper installation remains an explicit app/installer flow.
...
3. Privileged helper installation is not performed by this command yet.
4. For full closed-lid sleep prevention, use the in-app/helper installer flow once it is shipped.
...
echo "Privileged helper is not installed by this command yet."
INSTALL
