...
verify_release_readiness() {
  ...
  require_text 'v1\.5\.0 릴리즈 완료' "$DOC" "completed v1.5.0 release status"
  require_text 'b12dc69' "$DOC" "login item residual fix inclusion"
  require_text 'dirty branch PR/merge/delete 방지' "$DOC" "dirty branch release guard"
  ...
  require_text 'MacDog-1\.5\.0\.dmg' "$DOC" "v1.5.0 DMG artifact"
  require_text 'MacDog-1\.5\.0\.dmg\.sha256' "$DOC" "v1.5.0 checksum artifact"
  ...
  require_text '현재 GitHub Release는 \[v1\.5\.0\]' "$README" "README current v1.5.0 release"
  require_text '로그인 항목 선호 상태 hardening' "$README" "README v1.5.0 residual fix summary"
  require_text 'V150ReleaseReadiness\.md' "$ROADMAP" "ROADMAP release readiness doc link"
  require_text 'v1\.5\.0.*릴리즈 완료' "$ROADMAP" "ROADMAP v1.5.0 release completion"
  require_text 'git status --short.*PR 생성, PR merge, 로컬/원격 브랜치 삭제' "$AGENTS" "AGENTS dirty worktree PR/merge/delete guard"
  require_text '미커밋 변경은 금지하지 않습니다' "$AGENTS" "AGENTS uncommitted work allowance"
