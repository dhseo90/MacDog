...
verify_sources() {
  ...
  require_match 'resetCandidateConfirmationSeconds' "$HISTORY_SOURCE" "sustained reset candidate confirmation"
  require_match 'reconcileConfirmedSummaries' "$HISTORY_SOURCE" "confirmed completed-window reconciliation"
  ...
  require_match 'reset window history append: \(stored\|skipped\)' "$USAGE_FETCH_SMOKE" "reset-window append diagnostic live smoke guard"
  ...
  require_match 'testBackfillSummariesRequireSustainedRecoveryBeforeSplittingRollingResetTimestamps' "$HISTORY_TESTS" "reset boundary sustained-confirmation test"
  require_match 'testStoreReconcilesObservedRecordsWithConfirmedCompletedWindows' "$HISTORY_TESTS" "confirmed completed-window storage test"
