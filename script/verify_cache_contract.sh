...
require_match '\(CodexUsageResetWindowHistoryStore\.completedWindowRetentionCount \+ 1\) \* 7 \* 24 \* 60 \* 60' "$HISTORY_SOURCE"
require_match 'CodexUsageWeeklyHistorySample\(report:' "$CACHE_SOURCE"
require_match 'CodexUsageCacheWriteResult' "$CACHE_SOURCE"
require_match 'recordingStartedAt' "$CACHE_SOURCE"
...
require_match 'CodexUsageCacheWriteDiagnosticFormatter' "$CLI_SOURCE"
[[ -x "$USAGE_FETCH_SMOKE" ]] || die "usage fetch cache contract smoke is missing or not executable: $USAGE_FETCH_SMOKE"
...
require_match 'WriteSuccessRejectsInvalidCodexReport' "$CACHE_TESTS"
require_match 'FailureSnapshotDropsInvalidExistingReport' "$CACHE_TESTS"
...
require_match 'WriteSuccessAcceptsWeeklyOnlyReportAndSkipsFiveHourHistory' "$CACHE_TESTS"
require_match 'FullAndWeeklyOnlyTransitionsDoNotReuseStaleFiveHourWindowAndResumeHistory' "$CACHE_TESTS"
require_match 'FailureSnapshotPreservesWeeklyOnlyLastSuccessReport' "$CACHE_TESTS"
require_match 'WriteSuccessReturnsStoredWeeklyHistoryDiagnostic' "$CACHE_TESTS"
require_match 'WriteSuccessReturnsSkippedWeeklyHistoryDiagnostic' "$CACHE_TESTS"
require_match 'WeeklyHistorySkipsDenseUnchangedSamples' "$CACHE_TESTS"
require_match 'WeeklyHistoryRetainsThirteenWeeksForCompletedWindowRecovery' "$CACHE_TESTS"
require_match 'recordingStartedAt' "$USAGE_FETCH_SMOKE"
