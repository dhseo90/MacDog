...
INSTALL_STATE_SCRIPT="$ROOT_DIR/script/verify_install_state.sh"
RELEASE_FINAL_STATE_SCRIPT="$ROOT_DIR/script/verify_release_final_state.sh"
...
LOGIN_CONTROLLER_TESTS="$ROOT_DIR/Tests/MacDogTests/LoginLaunchControllerTests.swift"
LOGIN_COORDINATOR_TESTS="$ROOT_DIR/Tests/MacDogTests/LoginLaunchPreferenceCoordinatorTests.swift"
...
require_contains "$LOGIN_CONTROLLER_SOURCE" "LoginLaunchPreferenceCoordinator"
require_contains "$LOGIN_CONTROLLER_SOURCE" "loginItemUnavailable"
require_contains "$LOGIN_CONTROLLER_TESTS" "testEnablingFailsWhenMainAppLoginItemRemainsUnavailableAfterRegister"
require_contains "$LOGIN_COORDINATOR_TESTS" "testEnableFailureStoresDisabledPreference"
require_contains "$MAIN_SOURCE" "--set-login-item"
require_contains "$MAIN_SOURCE" "--verify-login-item-status"
require_contains "$INSTALL_STATE_SCRIPT" "login_item_status_for()"
require_contains "$INSTALL_STATE_SCRIPT" "login-item-status:"
require_contains "$INSTALL_STATE_SCRIPT" "expected login item status enabled because"
require_contains "$RELEASE_FINAL_STATE_SCRIPT" "login_item_status_for()"
require_contains "$RELEASE_FINAL_STATE_SCRIPT" "login item status mismatch"
