...
- `install.sh --with-helper`는 터미널이 있으면 `sudo`, 비대화형/더블클릭 흐름이면 macOS administrator dialog를 사용합니다.
...
- release staging의 `Check Install Status.command`는 앱, CLI, user LaunchAgent, optional helper 설치/로드 상태와 실행 중인 MacDog binary 경로를 읽기 전용으로 요약합니다.
...
- 2026-05-29에 이전 설치본 재발 방지를 위해 `verify_install_state.sh --expect-current-dist`와 release `Check Install Status.command`의 payload freshness guard를 추가했습니다.
...
2. GitHub Release용 더블클릭 설치 artifact의 실제 Finder 실행 경로를 검증합니다.
