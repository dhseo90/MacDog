...
상태: helper 우선 sleep 제어 코드 구현 / helper-only 실제 설치와 XPC read,set 검증 완료 / UI 설정 변경 검증 완료 / 장시간 덮개 닫힘 실사용 검증 통과 / 더블클릭 설치본 안내와 상태 확인 command 구성
작성일: 2026-05-28
...
- 잠들지 않기 탭은 helper 설치 상태를 읽기 전용으로 표시하고, 미설치/부분 설치/설치됨 상태별 다음 조치를 안내한다.
...
1. GitHub Release용 더블클릭 설치 artifact의 실제 Finder 실행 경로를 검증한다.
2. 앱 내부 helper 설치 UX를 별도 화면/상태로 polish한다.
