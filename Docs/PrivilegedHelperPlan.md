...
- release staging의 `Check Install Status.command`는 앱, CLI, user LaunchAgent, optional helper 설치/로드 상태를 읽기 전용으로 요약한다.
...
- `verify_install_state.sh --expect-current-dist`는 UI 검수 전에 설치된 앱과 `dist/MacDog.app`의 payload가 같은지 확인해 이전 설치본 실행 문제를 막는다.
