...
MacDog의 `pmset disablesleep` 제어와 macOS `screenLock` 지연 설정 제어를 AppleScript 관리자 프롬프트 반복 없이 처리합니다. 사용자는 helper 설치 때 한 번 승인하고, 이후 잠들지 않기 시간/상태 기준 변경은 앱 UI에서 자연스럽게 처리합니다.
...
- helper command allowlist를 `SleepDisabled` 조회, `SleepDisabled 0/1` 변경, `screenLock` 조회, `screenLock off/immediate/seconds` 변경으로 제한합니다.
...
- ad-hoc 개발 host는 bundle id만으로 허용하지 않고, 설치 시점의 host designated requirement를 `MACDOG_HELPER_HOST_REQUIREMENT`로 고정합니다.
...
- 앱 쪽 `MacDogPrivilegedHelperClient`는 XPC request/response codec을 공유하고, 덮개 닫힘 보호와 잠금 화면 암호 요구 해제의 helper 우선 제어 경로에 연결됩니다.
...
- 앱의 잠금 화면 암호 요구 해제는 helper가 설치된 상태면 helper 연결로 기존 `screenLock` 값을 저장하고 `screenLock off` 적용을 시도합니다. 적용 후 `sysadminctl -screenLock status`를 다시 확인하며, 실패하면 경고를 유지합니다.
- 덮개 닫힘 보호는 helper가 설치되지 않았을 때만 MacDog 앱이 소유한 관리자 승인 경로를 사용합니다. 잠금 화면 암호 요구 해제는 helper가 없으면 기존 `com.apple.screensaver` fallback만 시도하고 실제 `screenLock`이 남아 있으면 경고를 표시합니다. helper가 설치되어 있는데 연결이나 변경이 실패하면 예전 승인창으로 조용히 우회하지 않고 실패 상태를 표시합니다.
...
- 허용 명령은 `/usr/bin/pmset -g live`, `/usr/bin/pmset -a disablesleep 0`, `/usr/bin/pmset -a disablesleep 1`, `/usr/sbin/sysadminctl -screenLock status`, `/usr/sbin/sysadminctl -screenLock off`, `/usr/sbin/sysadminctl -screenLock immediate`, `/usr/sbin/sysadminctl -screenLock <seconds>`뿐입니다.
