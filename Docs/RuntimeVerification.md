...
상태: 절차 정리 완료 / 짧은 CPU/RSS/energy 검토 경로 보존 / 장시간 benchmark 미수행
작성일: 2026-05-27
...
```sh
MACDOG_APP_VERSION=<version> ./script/build_and_run.sh --verify-runtime 60
MACDOG_APP_VERSION=<version> ./script/build_and_run.sh --verify-floating-pet-runtime 60
```

더 긴 검증이 필요하면 같은 명령의 duration을 늘립니다.

```sh
MACDOG_APP_VERSION=<version> ./script/build_and_run.sh --verify-runtime 300
MACDOG_APP_VERSION=<version> ./script/build_and_run.sh --verify-floating-pet-runtime 300
```
