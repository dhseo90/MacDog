...
상태: 로드맵 초안 / Apple Developer 계정 사전 확인 반영
작성일: 2026-06-04
갱신일: 2026-06-05
기준 릴리즈: v1.2.3
작업 브랜치: `v.1.3.0`
...
2. 알림은 사용자가 설정 탭에서 켜고 macOS 권한을 승인한 뒤에만 발송합니다.
...
7. Apple Developer 계정이 필요한 기능명은 v1.3.0 문서/로드맵/검증 항목에 나열하지 않습니다.
8. Codex 사용량 JSON/cache/app-server 계약은 변경하지 않습니다.
...
Apple Developer 계정 필요성이 확인되면 전체 개발이 막히므로 가장 먼저 확인하고, 알림 정책과 설정 UI를 고정한 뒤 탭별 UI 개선으로 넘어갑니다.
...
- `ROADMAP.md`와 이 문서에 v1.3.0 범위, 제외 경계, 완료 기준을 기록합니다.
