...
| v1.5.0 (2) | 지난 사용량/reset boundary 그래프 회귀 수정 | weekly reset 이후 새 window와 Codex 탭 `지난` 사용량이 이전 32% marker 뒤로 이어지지 않고, 실제 reset으로 중단된 partial window를 backfill하며 같은 logical weekly window의 rolling reset timestamp record를 dedupe |
...
- OpenAI가 주간 한도를 실제 리셋해 `resetsAt`이 바뀐 경우 이전 history와 새 timeline을 분리합니다.
