...
| Script | 의미 | 영향 |
| --- | --- | --- |
| `script/verify_app_bundle.sh` | app bundle 구조 검증 | 지정 앱 번들의 앱, `codex-usage`, `macdog-claude-statusline`, helper, signature 구조를 읽고 기본 번들에 widget extension이 없는지 확인합니다. `--with-widget`은 opt-in `.appex` 포함을 확인합니다. |
| `script/verify_app_privacy_boundaries.sh` | 앱 privacy boundary 검증 | menu bar UI가 live Codex app-server나 shared cache fallback, Claude settings/auth/transcript/raw event를 직접 열지 않고 sanitized cache만 읽는지 코드 기준으로 확인합니다. |
...
| `script/verify_v191_multi_provider_contract.sh --self-test` | v1.9.1 복수 provider와 메인 provider 계약 자체검증 | `~/.grok/auth.json`, live billing, GUI, 설치, push 없이 selection, writer 생명주기, work-diff, 통합 게이지, graph-hidden screenshot, 하위 탭/합산/fallback/auth 거부와 문서 용어를 확인합니다. published `v1.9.1`을 주장하지 않습니다. |
| `script/verify_v191_release_readiness.sh --self-test` | v1.9.1 릴리즈 준비 계약 자체검증 | network, GUI, 설치, tag/workflow/publish 없이 체크리스트, published `v1.9.0` 유지, Finder/live/DMG 미수행 분리와 증거 기록 경계를 확인합니다. |
...
| `script/cleanup_release_smoke_state.sh --apply` | release smoke 잔여물 정리 | 마운트된 MacDog DMG를 eject하고, `~/Applications`·Desktop·모든 git worktree `dist`·`/private/tmp/macdog-*`의 중복 `MacDog.app`을 LaunchServices에서 unregister합니다. 중복 앱은 `/private/tmp/macdog-duplicate-app-cleanup.noindex` 아래 `.app.quarantined` 이름으로 격리하고, stale CLI symlink·usage cache LaunchAgent도 정리합니다. `/Applications/MacDog.app`은 이동하지 않습니다. |
