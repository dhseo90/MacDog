...
| 번호 | 우선순위 | 이슈 | 완료 증거 |
| ---: | --- | --- | --- |
| 1 | P0 | 원격 CI와 보호 규칙 상태 확인 | `v1.4.0` release head 기준 required checks 2개 통과, direct push bypass 기록 |
| 2 | P0 | v1.4.0 최종 릴리즈 문서 정리 | README/ROADMAP/Docs가 release head, DMG 이름, checksum, smoke 결과 기록 위치를 같은 용어로 설명 |
| 3 | P0 | 릴리즈 전 자동검증 | `git diff --check`, `markdownlint-cli2`, v1.4.0 self-test, 전체 `swift test`, Xcode Debug build, `MACDOG_RELEASE_VERSION=1.4.0 ./script/check.sh --no-run` 통과 |
| 4 | P0 | release candidate 패키징 | 원격 tag `v1.4.0` 없음, `MacDog-1.4.0.dmg`와 `MacDog-1.4.0.dmg.sha256` 생성, checksum과 `hdiutil verify` 통과 |
