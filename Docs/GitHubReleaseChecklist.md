...
- `.github/workflows/ci.yml`: PR과 `main` push에서 `./script/check.sh --no-run`을 실행합니다.
...
- README 스크린샷을 갱신한 경우 임시 이미지가 남지 않았고 README renderer 산출물과 커밋 이미지가 일치하는지 `script/verify_readme_screenshots.sh`로 확인합니다.
