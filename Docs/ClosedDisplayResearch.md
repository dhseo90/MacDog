...
- 2026-05-28 이전 설치본이 실행 중이면 helper 연동 이전 경로가 남아 비밀번호 프롬프트가 발생할 수 있음을 확인했습니다. UI 검수 전 `./script/verify_install_state.sh --expect-current-dist` 또는 release `Check Install Status.command`로 설치본과 현재 payload 일치를 확인해야 합니다.
