...
- 2026-06-02 sandbox 밖 사용자 세션에서 `/usr/bin/shortcuts list`가 정상 종료했고, `script/verify_shortcuts_charge_limit.sh`는 `shortcuts:available count=0`, `charge-limit-shortcuts:candidates count=0`을 출력했습니다. 현재 Shortcuts library가 비어 있어 Charge Limit 후보 액션 이름과 입력 계약은 캡처하지 못했습니다.
...
5. Shortcuts 액션은 실제 대상 OS에서 Charge Limit 후보 단축어/액션이 존재할 때 액션 이름과 입력 계약을 확인한 뒤 별도 기능으로 붙입니다. 현재 표준 체크는 후보 이름 파서를 local fixture로 검증하고, Shortcuts CLI/helper 사용 가능 여부와 Charge Limit 후보 단축어 이름만 read-only로 확인합니다.
...
- native Charge Limit 회귀 검증 유지: macOS 업데이트, 공개 배포 설치본 변경, helper/앱 재설치 이후 `charge-limit current`와 Battery settings 표시가 계속 일치하는지 재확인
- Shortcuts CLI/helper가 사용 가능하고 Charge Limit 후보 단축어/액션이 존재하는 환경에서 후보 이름과 입력 계약 확인
