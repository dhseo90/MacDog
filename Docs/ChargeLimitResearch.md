...
상태: native Charge Limit 직접 제어 1차 구현 / SMC 직접 제어는 별도 기능으로 검토
작성일: 2026-05-27
갱신일: 2026-05-28
...
- 2026-05-28 같은 값 `95`로 set/restore 검증을 수행해 native Charge Limit 쓰기 경로가 동작함을 확인했다. 실제 90% 변경은 장시간 덮개 닫힘 테스트 때 별도로 수행한다.
...
- UI에서 90%로 실제 적용한 뒤 장시간 덮개 닫힘 테스트와 함께 충전 한도가 90%로 유지/하강하는지 확인
