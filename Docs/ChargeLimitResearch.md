...
4. privileged helper는 먼저 덮개 닫힘 보호의 반복 관리자 프롬프트 제거용으로 도입한다.
5. 배터리 충전 제어는 helper 통로를 재사용할 수 있지만, native Charge Limit/Shortcuts 경로 검증을 먼저 한다.
6. SMC 방식이 필요하면 uninstall 원복, 실패 복구, macOS 업데이트 호환성, 배터리 calibration 리스크를 별도 기능 게이트로 설계한다.
