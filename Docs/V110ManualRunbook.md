...
실제 UI, 설치, signing, GitHub Actions를 건드리기 전 read-only 상태를 확인합니다.
...
`./script/verify_v110_manual_execution_readiness.sh --allow-incomplete`는 실제 실행 없이 7개 우선 항목이 현재 `ready-for-manual-ui`, `blocked`, `external-required`, `ready-for-additional-runtime-sampling` 중 어디에 있는지 요약합니다.
...
확인된 범위는 `Docs/WidgetPackaging.md`에 남깁니다. 요약하면 source-level WidgetKit view/provider, small/medium presentation, deep link 상수, cache 상태 표현, fixture writer self-test, Xcode host/extension build는 일부 확인됐지만, 실제 위젯 UI의 shared cache 읽기, stale/error 반영, 클릭 deep link는 App Group provisioning이 되는 opt-in build에서만 다시 검수합니다.
...
Evidence id: `signed_stable_helper_ux`
...
- Developer ID signed stable DMG 산출물과 checksum을 확인합니다.
- signed build에서 helper 설치 승인창 주체가 MacDog로 표시되는지 확인합니다.
- helper 설치/제거 경로와 사용자 안내 문구가 stable artifact 기준으로 명확한지 확인합니다.

기록 예:

```sh
script/record_v110_manual_evidence.sh --item signed_stable_helper_ux --status verified --evidence "signed stable DMG <path/checksum>에서 helper 승인창 주체 MacDog, 설치/제거 UX 확인"
```
...
Evidence id: `github_actions_release_run`
...
- draft release workflow run URL과 결과를 기록합니다.
- stable release workflow run URL과 결과를 기록합니다.
- 생성된 artifact, checksum, GitHub Release 결과를 기록합니다.
- run URL은 `https://github.com/<owner>/<repo>/actions/runs/<run-id>` 형태의 실제 URL로 기록합니다.
- GitHub Release 결과는 생성된 release 또는 tag URL을 함께 기록합니다.

기록 예:

```sh
script/record_v110_manual_evidence.sh --item github_actions_release_run --status verified --evidence "release candidate workflow run URL https://github.com/dhseo90/MacDog/actions/runs/<id> success, draft release workflow run URL https://github.com/dhseo90/MacDog/actions/runs/<id> success, stable release workflow run URL https://github.com/dhseo90/MacDog/actions/runs/<id> success, artifact MacDog-<version>.dmg, checksum MacDog-<version>.dmg.sha256, GitHub Release https://github.com/dhseo90/MacDog/releases/tag/<tag>"
```

## 5. Developer ID signing, notarization, stapling, Gatekeeper 검증

Evidence id: `developer_id_distribution_gate`
...
- Developer ID signing에 사용한 stable artifact 식별자와 checksum을 기록합니다.
- `xcrun notarytool submit` 성공 결과를 기록합니다.
- `xcrun stapler staple`과 `xcrun stapler validate` 성공 결과를 기록합니다.
- `spctl` Gatekeeper assessment 성공 결과를 기록합니다.
- checksum은 64자 SHA-256 값을 기록합니다.
- `spctl` 결과는 `accepted` 또는 `source=Notarized Developer ID` 같은 Gatekeeper 판정 문구를 함께 기록합니다.

기록 예:

```sh
script/record_v110_manual_evidence.sh --item developer_id_distribution_gate --status verified --evidence "Developer ID signed MacDog-<version>.dmg checksum <64-char-sha256>, notarytool submit Accepted, stapler staple worked, stapler validate accepted, spctl --assess accepted source=Notarized Developer ID"
```
...
- GUI 앱 실행, 설치/삭제, LaunchAgent 등록, signing, notarization, stapling, Gatekeeper 검증, GitHub Actions 실행은 각 항목을 수행하기로 명시했을 때만 진행합니다.
- 실행하지 않은 UI, 설치, workflow, signing 결과를 `확인됨`으로 기록하지 않습니다.
