...
- DMG 안에는 `MacDog.app`과 `Applications` symlink를 넣어 표준 drag-and-drop 설치 경험을 제공합니다.
- 로컬 검증용으로는 `Install MacDog.command`, `Uninstall MacDog.command`, `Check Install Status.command`를 함께 제공합니다.
- optional 권한 도우미 설치/제거는 별도 helper `.command`가 아니라 MacDog 설정 탭에서 처리합니다.
- 공개 배포 전에는 Developer ID signing, hardened runtime, notarization, stapling, Gatekeeper 검증을 별도 gate로 둡니다.

## 현재 구현 범위

- `script/package_release.sh --dry-run`은 release artifact 계획을 출력합니다.
- `script/package_release.sh`는 앱 번들 내부 CLI가 포함된 `dist/MacDog.app`을 `dist/release/MacDog-<version>`에 staging합니다.
- staging 폴더에는 `MacDog.app`, `Applications` symlink, `Install MacDog.command`, `Uninstall MacDog.command`, `Check Install Status.command`, `README_FIRST.txt`, `RELEASE_NOTES_DRAFT.md`가 포함됩니다.
- drag 설치 후 앱을 처음 열면 MacDog가 번들 내부 `codex-usage`를 짧은 cache writer로 실행해 app-owned usage cache를 채울 수 있습니다.
- `Install MacDog.command`는 로컬 검증용 full install 경로입니다. 더블클릭하면 사용자 영역에 앱, 앱 번들 내부 CLI를 가리키는 터미널용 symlink, user LaunchAgent를 설치하고 앱을 엽니다.
- 메뉴바 앱 자동 실행 monitor LaunchAgent는 `loginLaunchEnabled` 설정이 켜져 있을 때만 등록합니다.
- `Uninstall MacDog.command`는 사용자 영역의 앱, 터미널용 CLI symlink, LaunchAgent, cache 파일을 제거하며 optional 권한 도우미는 건드리지 않습니다.
- `Check Install Status.command`는 앱, 앱 번들 내부 CLI, 터미널용 symlink, user LaunchAgent, optional helper 설치/로드 상태, 설치된 앱이 DMG payload와 같은 빌드인지, 실행 중인 MacDog가 다른 binary인지 터미널에서 요약합니다.
- 설치/업데이트 중 MacDog가 `SleepDisabled=1`을 소유한 상태라면 app 종료 정리 루틴이 값을 0으로 되돌리지 않도록 강제 종료 경로를 사용합니다.
...
- `script/verify_release_packaging.sh`는 dry-run 문구, staging payload 구조, Applications symlink, release note draft, installer/uninstaller syntax, LaunchAgent plist heredoc 구조, usage cache cleanup, helper 별도 command 제거, command 파일의 `osascript` 승인창 미사용, 설치 후 상태 확인 command의 freshness smoke를 검증합니다.
...
- optional helper 설치/제거는 앱 설정 탭으로 안내합니다.
...
3. 생성된 `dist/release/MacDog-<version>.dmg`를 열어 `MacDog.app`과 `Applications` symlink를 확인합니다.
4. Finder에서 `MacDog.app`을 `Applications`로 드래그해 설치합니다.
5. MacDog를 실행하고 Codex 사용량 cache가 채워지는지, 설정 탭에서 로그인 자동 실행, 데스크톱 펫, 권한 도우미 상태가 보이는지 확인합니다.
6. 터미널용 CLI symlink와 user LaunchAgent까지 검증해야 하면 `Install MacDog.command`를 실행합니다.
7. `Check Install Status.command`로 app/번들 내부 CLI/터미널 symlink/LaunchAgent/helper 상태와 실행 중인 MacDog binary 경로를 확인합니다.
8. `shasum -a 256 -c dist/release/MacDog-<version>.dmg.sha256`로 checksum을 확인합니다.
9. 제거 검증이 필요하면 앱 설정 탭에서 optional helper를 먼저 제거하고, `Uninstall MacDog.command`로 user component를 제거합니다.
...
- helper 설치가 포함되는 경우 앱 설정 탭이 `/Library/PrivilegedHelperTools`와 `/Library/LaunchDaemons` 변경을 명확히 안내하고 uninstall 복구를 검증합니다.
