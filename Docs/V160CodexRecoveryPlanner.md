...
- 기존 Codex 탭의 현재 위험 기준과 알림 기준 요약은 compact하게 유지합니다.
...
- 주간 잔여량 그래프는 보조 정보로 낮춰 Codex 탭 하단에 배치하되, `현재`/`지난`/`비교` history 모드 전환과 PNG 복사/내보내기 액션은 유지합니다.
...
- 현재 위험 기준과 알림 기준 요약이 1번 탭 안에 유지됩니다.
- 초기화권 영역은 보유 장수와 각 장의 유효기간을 표시합니다.
- history 영역은 `현재`/`지난`/`비교` 모드를 1번 탭 안에서 접힘 없이 표시합니다.
