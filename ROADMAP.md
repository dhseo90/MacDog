...
| 8 | 배포와 설치 | 자동 검증 완료, 로컬 `.dmg` 후보 생성/검증 완료, 더블클릭 설치/제거 command, 상태 확인 command, checksum, release note draft 구성, unsigned draft release workflow 구성 | GitHub Actions 실제 실행, 깨끗한 사용자 환경의 더블클릭 설치/제거 검수, 공개 배포 signing/notarization |
