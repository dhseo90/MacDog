...
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, 권한 도우미 연결 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증, 장시간 덮개 닫힘 실사용 검증 완료, 더블클릭 helper 설치/제거 안내와 상태 확인 command 구성, 앱 내부 helper 설치/제거 버튼 실제 클릭 검수, helper 버튼 상태별 자동 검증, UI 검수 전 설치본 freshness guard와 수동 UI 검수 preflight 구성, 공개 배포용 helper installer gate 구성 | signed stable DMG helper 설치 실제 검수 |
...
- 앱 권한 팝업 재발 확인: 2026-05-29 최신 설치본에서 `macdog://open` 탭 전환은 문제 없었지만, 플로팅 펫 좌클릭 재검수 중 `codex-usage` App Data 권한 팝업을 재현했다. `PopoverTabArtwork`의 launch cwd 리소스 탐색을 제거하고, `codex app-server` 실행 cwd를 `/tmp`로 고정한 뒤 최신 설치본에서 플로팅 펫 좌클릭을 다시 검수했다. popover는 강아지 옆에 열렸고 macOS "다른 앱의 데이터 접근" 팝업은 재발하지 않았다. `verify_manual_ui_prerequisites.sh`, `verify_cache_contract.sh`, `./script/check.sh --no-run`도 통과했다.
- 캐릭터 artwork polish: 2026-05-29 탭 버튼 artwork를 현재 `Codex Pup` 데스크톱 sprite 기반으로 더 단순하게 재렌더링하고 README fake data 스크린샷을 갱신했다.
...
6. 탭 버튼 artwork 1차 polish: 강아지 중심 배치, 우상단 주제 배지, README fake data 스크린샷 갱신
