...
아래 목록은 2026-05-29 기준 실제 남은 작업이다. 구현 완료와 자동 검증 완료 항목은 잔여 이슈에서 제외하고, 수동 UI 검수나 외부 배포 gate처럼 아직 증거가 부족한 항목만 남긴다.

완료된 항목:

- 앱 권한 팝업 재발 확인: 2026-05-29 최신 설치본에서 `macdog://open` 탭 전환은 문제 없었지만, 플로팅 펫 좌클릭 재검수 중 `codex-usage` App Data 권한 팝업을 재현했다. `PopoverTabArtwork`의 launch cwd 리소스 탐색을 제거하고, `codex app-server` 실행 cwd를 `/tmp`로 고정한 뒤 최신 설치본에서 플로팅 펫 좌클릭을 다시 검수했다. popover는 강아지 옆에 열렸고 macOS "다른 앱의 데이터 접근" 팝업은 재발하지 않았다. 이후 5분 cache LaunchAgent가 단독 `/Users/dhseo/bin/codex-usage` 실행 파일을 주체로 app-server를 호출하는 구조도 확인되어, 설치/릴리즈 경로를 앱 번들 내부 `Contents/MacOS/codex-usage` 실행과 `~/bin/codex-usage` symlink 구조로 정리했다.
- 캐릭터 artwork polish: 2026-05-29 탭 버튼 artwork를 현재 `Codex Pup` 데스크톱 sprite 기반으로 더 단순하게 재렌더링했다. README 전용 스크린샷은 `Docs/Images/README` 아래의 문서 리소스로 관리한다.

남은 항목:

1. 앱 내부 helper 버튼 실제 클릭 검수: `잠들지 않기` 탭의 helper 설치/제거 버튼을 실제 UI에서 눌러 상태 전환과 안내 문구를 확인한다.
2. 깨끗한 더블클릭 설치본 검수: 오래된 설치본과 stale `dist` 앱 복사본이 없는 상태에서 `.dmg`의 설치/제거/status command를 확인한다.
3. WidgetKit 수동 검수: 위젯 갤러리 추가, 위젯 클릭 deep link, stale/error 표시를 실제 macOS UI에서 확인한다.
4. 플로팅 펫 실제 동작 검수: 좌클릭 popover와 말풍선 방향은 최신 설치본에서 확인했다. 남은 검수는 드래그 위치 저장, 우클릭 메뉴, 화면 밖 보정, 메뉴바와의 action 차이다.
5. Shortcuts Charge Limit 입력 계약 확인: Shortcuts helper가 정상 동작하는 환경에서 액션 이름과 입력 계약을 확인한다.
6. GitHub 배포 gate 실제 실행: unsigned draft artifact, signed/notarized stable workflow, Gatekeeper 검증을 실제 GitHub/Developer ID 환경에서 확인한다.

후순위 별도 승인 항목:

- SMC 충전 제어는 native Charge Limit 안정화와 helper gate 이후 별도 실험 기능으로만 진행한다.
- 사용량 history, 알림, custom runner import는 확장 기능 milestone에서 다룬다.
...
현재 우측 탭 버튼, 플로팅 펫, 메뉴바 러너 프레임은 `MacDogCharacterProfile.codexPup`이 소유하는 하나의 캐릭터 프로필로 연결되어 있다. 우측 탭 버튼은 실제 데스크톱 펫 sprite에 탭별 주제 배지를 합성해 생성하며, 생성 원본/배지/출력 이름은 `codex-pup-tab-art.json` manifest와 Swift 프로필 테스트가 함께 고정한다. 2026-05-29에 탭 버튼 artwork는 강아지를 중심에 더 크게 두고 탭 주제 배지를 우상단에 얹는 1차 polish를 적용했다. 후속 캐릭터를 추가할 때는 버튼 이미지, 플로팅 펫, 메뉴바 러너 프레임을 각각 따로 고르지 않고 하나의 asset set으로 교체한다.
...
- monitor LaunchAgent는 `RunAtLoad`로 앱을 로그인/재시작 후 자동 실행한다. 사용자는 5번 설정 탭의 `로그인 시 MacDog 실행`으로 monitor LaunchAgent만 끌 수 있고, 앱은 `UserDefaults`의 기존 설정을 다시 읽어 잠자기 방지 모드와 자동 조건을 복원한다.
...
- `.dmg` 안에는 앱 번들 내부 `codex-usage`를 포함한 `MacDog.app`, `Applications` symlink, 로컬 검증용 설치/제거 command, 설치 상태 확인 command, 안내 문서를 포함한다.
- CLI와 LaunchAgent는 로컬 검증용 설치 command 또는 앱 설정에서 명확히 제어하고, privileged helper는 앱 설정 탭에서 명확히 승인받는다.
...
- 공개 배포 DMG는 helper 설치 command를 따로 넣지 않고, signed `MacDog.app`의 설정 탭에서 helper 설치/제거 승인을 받는다.
...
- 삭제 시 cache/LaunchAgent/terminal symlink 정리가 가능하다.
...
- 자동 trigger 1차 확장은 전원 연결, Codex 실행 중, 배터리 기준값 이상, CPU 사용량 기준값 이상, 메모리 사용량 기준값 이상, 네트워크 전송 중, 외장/공유 드라이브 연결 조건을 OR 조건으로 제공한다.
- trigger 세부 기준은 선택한 조건에 맞는 배터리 기준값, CPU 사용량 기준값, 메모리 사용량 기준값만 UI에서 조정한다. 네트워크 기준은 MacDog 기본값을 사용한다.
- 설정 탭은 화면 잠자기 방지, 덮개 닫힘 보호, 화면 보호기 이후 암호 요구 해제, 권한 도우미, 캐릭터/러너 표시 옵션을 한곳에서 제어한다.
- 덮개 닫힘 보호는 관리자 승인 후 `pmset disablesleep`을 켜고, MacDog가 켠 값만 끄기/시간 만료/조건 해제 시 되돌린다.
- privileged helper가 설치된 최신 설치본에서는 `잠들지 않기` UI의 `시간 제어`/`끔` 클릭이 권한 도우미 연결로 `SleepDisabled`를 변경하며, 설정 변경 중 비밀번호 프롬프트가 반복되지 않는 것을 확인했다.
...
- Shortcuts 기반 충전 제어 검증은 환경 이슈로 후속 작업으로 남기며, 현재 read-only probe는 후보 단축어 이름까지만 감지한다.
- 상태별 강아지 반응 1차는 CPU/메모리 과부하, 배터리 낮음, 충전 중 상태를 기존 alert/rest/idle 포즈로 표현하고 로밍을 잠시 멈춘다.
...
- 덮개를 덮었을 때 잠자기 방지는 관리자 승인 기반 1차 구현으로 다룬다.
  ...
  - 현재 구현은 `pmset -g live`의 `SleepDisabled`를 확인하고, helper가 설치되어 있으면 권한 도우미 연결로 `pmset disablesleep`을 변경한다.
  - 시간 만료/끄기/조건 해제 시 MacDog가 켠 전역 설정만 되돌린다.
...
- AlDente식 SMC 충전 제한 직접 제어는 별도 기능 게이트로 분리한다.
  ...
  - SMC 방식은 충전 되돌림, uninstall, macOS 업데이트 호환성, 배터리 calibration 리스크를 문서화한 뒤 별도 승인으로만 진행한다.
...
- 최신 설치본 UI에서 `시간 제어`/`끔`을 클릭하면 권한 도우미 연결로 `SleepDisabled`가 `1`/`0`으로 바뀌는 것을 확인한다.
...
- SMC 방식은 입력 범위, 충전 되돌림, 충돌 감지, hardware/OS 지원 매트릭스, uninstall 복구를 갖춘 뒤 별도 실험 기능으로만 추가한다.

잔여 리스크:

- helper는 서명/권한/설치 경로가 민감하므로 개인 사용, 외부 배포, App Store 배포 가능 범위를 분리해야 한다.
- 설치 실패 또는 helper version mismatch가 발생하면 사용자에게 되돌릴 수 있는 상태를 보여줘야 한다.
- `pmset disablesleep`은 전역 시스템 설정이므로 MacDog가 켠 값만 소유하고 되돌려야 한다.
...
- `PetMenuModel`이 메뉴바와 플로팅 펫 우클릭 메뉴의 공통 `PetAction` 계약을 제공하고, `PetMenuModelTests`가 표면별 메뉴 차이를 전환 항목으로만 제한한다.
