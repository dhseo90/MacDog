...
상단에 활성 provider 게이지 카드를 함께 표시합니다. 상세 그래프, 초기화권, 러너,
tooltip, 알림, 펫·잠들지 않기 문구는 메인 provider만 기준으로 합니다. Codex 탭에는
페이스메이커 블록을 두지 않고, Grok 메인은 주간 pace 요약을 둡니다.
...
4. 1번 탭 상단에 main 전체 게이지와 보조 provider 주간 게이지를 함께 표시합니다.
   `CombinedUsageGauges` focused test를 통과했습니다. provider 하위 탭은 넣지 않았습니다.
5. 상세 그래프 표시를 끄면 main 그래프 control과 plot만 숨기고 모든 게이지는 유지합니다.
   `UsageTabSectionVisibility` focused test를 통과했습니다. Codex 초기화권은 남기고
   페이스메이커 블록은 탭에 두지 않습니다.
6. controller task 전환, screenshot renderer, focused test, 전체 test와 Xcode build를
   순서대로 닫습니다. `UsageProviderWorkDiff`와
   `testUsagePopoverRendersSingleCodexMainDualGrokMainDualAndGraphHiddenStates`,
...
- 메뉴바에서는 움직임을 우선하고, 메인 provider 주간 잔여율 `%`만 아이콘 옆에 둡니다.
- 기본 상태는 방해되지 않아야 합니다.
