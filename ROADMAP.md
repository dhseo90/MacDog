...
1. 설치 UX polish: helper 포함 설치 흐름, 더블클릭 설치본 권한 안내, 설치 후 상태 확인 UX
2. 공개 배포 준비: GitHub Release artifact 정리, signing/notarization 여부 결정, Gatekeeper 경고 최소화
3. WidgetKit 수동 검수: 위젯 갤러리 추가, 클릭/deep link, stale/error 표시 확인
4. 캐릭터 asset 통일: 데스크톱 강아지, 메뉴바 러너, 오른쪽 탭 버튼을 같은 캐릭터 팩으로 묶기
   - 현재 오른쪽 탭 버튼은 버튼용 임시 생성 artwork이며, 최종 캐릭터 통일 완료로 보지 않는다.
5. SMC 충전 제어는 helper 안정화 이후 별도 승인 기능으로 분리
