...
| 8 | 배포와 설치 | 자동 검증 완료, 로컬 `.dmg` 후보 생성/검증 완료 | 깨끗한 사용자 환경의 더블클릭 설치 검수, 공개 배포 signing/notarization |
| 9 | MacDog 리브랜딩 | 구현 완료, 자동 검증 완료 | 기존 사용자 migration 필요 시 별도 처리 |
| 10 | MacDog 시스템 유틸리티 모듈 | RunCat식 상세 Mac 상태/배터리/잠자기 방지 제어 UI/자동 trigger 1차 확장/덮개 닫힘 보호/Charge Limit 연구 스파이크와 설정 연결, 장시간 덮개 닫힘/90% 충전 한도 실사용 검증 완료 | 활성 자원 CPU/RAM 그래프, trigger 세부 설정, Shortcuts 검증 |
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, XPC 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증, 장시간 덮개 닫힘 실사용 검증 완료 | 설치 UX polish, 공개 배포용 helper installer gate |
...
- GitHub Actions 수동 workflow는 unsigned `.dmg` 후보를 artifact로 보관하는 범위까지 준비한다.
- `.dmg` 안에는 `MacDog.app`, `codex-usage`, 더블클릭 설치 command, 안내 문서를 포함한다.
- CLI, LaunchAgent, privileged helper가 필요한 기능은 앱 내부 설치 화면이나 별도 installer flow에서 명확히 승인받는다.
- 외부 배포 전에는 Developer ID signing, hardened runtime, notarization, Gatekeeper 검증을 별도 milestone gate로 둔다.
...
1. Mac 활성 자원 탭 그래프화: CPU/RAM 최근 흐름을 30~60초 history로 표시하고, popover에 불필요한 스크롤이 생기지 않게 UI 배치 또는 창 크기를 조정한다.
2. 배터리 탭 polish: 90% 실사용 검증 결과를 기준으로 현재 한도, 목표 한도, 충전 상태, "즉시 방전 아님" 안내를 정리한다.
3. 잠자기 방지 고급 trigger 설정: threshold 사용자 조정, 지정 앱 선택, trigger별 종료 조건
4. Amphetamine식 잠자기 방지 세션 상세 polish와 display sleep/screen saver/lock 관련 옵션 정리
5. 설치 UX polish: helper 포함 설치 흐름, 더블클릭 설치본 권한 안내, 설치 후 상태 확인 UX
6. 공개 배포 준비: GitHub Release artifact 정리, signing/notarization 여부 결정, Gatekeeper 경고 최소화
7. WidgetKit 수동 검수: 위젯 갤러리 추가, 클릭/deep link, stale/error 표시 확인
8. 캐릭터 asset 통일: 데스크톱 강아지, 메뉴바 러너, 오른쪽 탭 버튼을 같은 캐릭터 팩으로 묶기
   - 현재 오른쪽 탭 버튼은 버튼용 임시 생성 artwork이며, 최종 캐릭터 통일 완료로 보지 않는다.
9. SMC 충전 제어는 helper 안정화 이후 별도 승인 기능으로 분리
