...
| 8 | 배포와 설치 | 자동 검증 완료, 로컬 `.dmg` 후보 생성/검증 완료, 더블클릭 설치 command/상태 확인 command 구성 | 깨끗한 사용자 환경의 더블클릭 설치 검수, 공개 배포 signing/notarization |
| 9 | MacDog 리브랜딩 | 구현 완료, 자동 검증 완료 | 기존 사용자 migration 필요 시 별도 처리 |
| 10 | MacDog 시스템 유틸리티 모듈 | RunCat식 상세 Mac 상태/배터리/잠자기 방지 제어 UI/자동 trigger 1차 확장/trigger 세부 기준/세션 옵션/덮개 닫힘 보호/Charge Limit 연구 스파이크와 설정 연결, 장시간 덮개 닫힘/90% 충전 한도 실사용 검증 완료 | Shortcuts 검증 |
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, XPC 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증, 장시간 덮개 닫힘 실사용 검증 완료, 더블클릭 helper installer 안내/상태 확인 command 구성 | 공개 배포용 helper installer gate |
...
- `.dmg` 안에는 `MacDog.app`, `codex-usage`, 더블클릭 설치 command, helper 설치 command, 설치 상태 확인 command, 안내 문서를 포함한다.
...
1. 공개 배포 준비: GitHub Release artifact 정리, signing/notarization 여부 결정, Gatekeeper 경고 최소화
2. WidgetKit 수동 검수: 위젯 갤러리 추가, 클릭/deep link, stale/error 표시 확인
3. 캐릭터 asset 통일: 데스크톱 강아지, 메뉴바 러너, 오른쪽 탭 버튼을 같은 캐릭터 팩으로 묶기
   - 현재 오른쪽 탭 버튼은 버튼용 임시 생성 artwork이며, 최종 캐릭터 통일 완료로 보지 않는다.
4. SMC 충전 제어는 helper 안정화 이후 별도 승인 기능으로 분리
