...
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드와 XPC 검증 발판 구현, 실제 설치 미실행 | 사용자 승인 후 실제 설치/삭제와 XPC 제어 검증, 실기 검증 |
...
- 덮개를 덮었을 때 잠자기 방지는 관리자 승인 기반 1차 구현으로 다룬다.
  ...
  - 현재 구현은 `pmset -g live`의 `SleepDisabled`를 확인하고, 필요 시 관리자 승인으로 `pmset disablesleep`을 변경한다.
