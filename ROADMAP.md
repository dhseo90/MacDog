...
| 10 | MacDog 시스템 유틸리티 모듈 | Mac 상태/배터리/잠자기 방지 세션/trigger/Charge Limit 지원 감지와 설정 연결 구현 완료 | RunCat식 상세 모니터, Amphetamine식 세션 UI, 충전 제어 연구 스파이크 |
...
- 배터리 읽기 전용 상태로 배터리 비율, 충전 여부, 전원 연결 여부, 완충/방전 예상 시간을 표시한다.
- 일반 잠자기 방지는 IOKit power assertion으로 시스템 idle sleep 방지 토글을 제공한다.
- 잠자기 방지 세션은 계속, 30분, 1시간, 2시간 preset을 제공하고 만료 시 자동으로 꺼진다.
- 조건부 잠자기 방지 trigger MVP는 전원 연결 중, Codex 앱 실행 중 조건을 지원한다.
...
1. RunCat식 상세 시스템 모니터 패널 고도화
2. 잠자기 방지 4대 세션 UI 정리: 항상 금지, 충전 중 금지, 시간 기준 금지, 앱 실행 중 금지
3. Amphetamine식 잠자기 방지 세션 상세/trigger 확장
4. 덮개 닫힘/closed-display mode 가능성 조사
