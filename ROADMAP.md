...
1. 앱 권한 팝업 재발 확인: 설치본이 앱 소유 cache만 읽는 상태에서 macOS "다른 앱의 데이터 접근" 팝업이 다시 뜨지 않는지 최신 UI로 확인한다.
2. 앱 내부 helper 버튼 실제 클릭 검수: `잠들지 않기` 탭의 helper 설치/제거 버튼을 실제 UI에서 눌러 상태 전환과 안내 문구를 확인한다.
3. 깨끗한 더블클릭 설치본 검수: 오래된 설치본과 stale `dist` 앱 복사본이 없는 상태에서 `.dmg`의 설치/제거/status command를 확인한다.
4. WidgetKit 수동 검수: 위젯 갤러리 추가, 위젯 클릭 deep link, stale/error 표시를 실제 macOS UI에서 확인한다.
5. 플로팅 펫 실제 동작 검수: 드래그 위치 저장, 좌/우 클릭 패널, 화면 밖 보정, 메뉴바와의 action 차이를 실제 화면에서 확인한다.
6. 캐릭터 artwork polish: 탭 버튼, 데스크톱 펫, 메뉴바 러너가 같은 `Codex Pup` 세트로 보이도록 시각 톤을 더 맞춘다.
7. Shortcuts Charge Limit 입력 계약 확인: Shortcuts helper가 정상 동작하는 환경에서 액션 이름과 입력 계약을 확인한다.
8. GitHub 배포 gate 실제 실행: unsigned draft artifact, signed/notarized stable workflow, Gatekeeper 검증을 실제 GitHub/Developer ID 환경에서 확인한다.
