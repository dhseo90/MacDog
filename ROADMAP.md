...
| 10 | MacDog 시스템 유틸리티 모듈 | RunCat식 상세 Mac 상태/배터리/잠자기 방지 제어 UI/자동 trigger 1차 확장/덮개 닫힘 보호/Charge Limit 연구 스파이크와 설정 연결 구현 완료 | trigger 세부 설정, Shortcuts 검증, 장시간 덮개 닫힘 검증 |
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, XPC 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증 완료 | 설치 UX polish, 공개 배포용 helper installer gate, 장시간 덮개 닫힘 검증 |
...
1. Privileged Helper 설치 통합으로 `pmset disablesleep` 반복 관리자 프롬프트 제거
2. 잠자기 방지 고급 trigger 설정: threshold 사용자 조정, 지정 앱 선택, trigger별 종료 조건
