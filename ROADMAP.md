...
- 앱 권한 팝업 재발 확인: 2026-05-29 최신 설치본에서 `macdog://open`으로 popover를 열고 Codex, Mac, 잠들지 않기, 배터리 탭을 실제 클릭했다. macOS "다른 앱의 데이터 접근" 팝업은 재발하지 않았고, `verify_manual_ui_prerequisites.sh`도 설치본 freshness와 privacy boundary를 통과했다.
...
4. 플로팅 펫 실제 동작 검수: 드래그 위치 저장, 좌/우 클릭 패널, 화면 밖 보정, 메뉴바와의 action 차이를 실제 화면에서 확인한다.
