...
| 10 | MacDog 시스템 유틸리티 모듈 | RunCat식 상세 Mac 상태/배터리/잠자기 방지 제어 UI/자동 trigger 1차 확장/trigger 세부 기준/덮개 닫힘 보호/Charge Limit 연구 스파이크와 설정 연결, 장시간 덮개 닫힘/90% 충전 한도 실사용 검증 완료 | Amphetamine식 세션 상세 polish, Shortcuts 검증 |
...
- display sleep, screen saver, lock 정책처럼 Amphetamine식 세션 상세 옵션은 후속 작업으로 남긴다.
...
1. Amphetamine식 잠자기 방지 세션 상세 polish와 display sleep/screen saver/lock 관련 옵션 정리
2. 설치 UX polish: helper 포함 설치 흐름, 더블클릭 설치본 권한 안내, 설치 후 상태 확인 UX
3. 공개 배포 준비: GitHub Release artifact 정리, signing/notarization 여부 결정, Gatekeeper 경고 최소화
4. WidgetKit 수동 검수: 위젯 갤러리 추가, 클릭/deep link, stale/error 표시 확인
5. 캐릭터 asset 통일: 데스크톱 강아지, 메뉴바 러너, 오른쪽 탭 버튼을 같은 캐릭터 팩으로 묶기
   - 현재 오른쪽 탭 버튼은 버튼용 임시 생성 artwork이며, 최종 캐릭터 통일 완료로 보지 않는다.
6. SMC 충전 제어는 helper 안정화 이후 별도 승인 기능으로 분리
