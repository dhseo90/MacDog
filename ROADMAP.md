...
| 6 | 데스크톱 플로팅/로밍 펫 | 1차 구현 완료, 자동 검증 완료 | 우클릭 패널 위치, 드래그/로밍 충돌 개선 |
...
| 10 | MacDog 시스템 유틸리티 모듈 | Mac 상태/배터리/잠자기 방지 세션/trigger/Charge Limit 지원 감지와 설정 연결 구현 완료 | 실제 충전 한도 제어 API 확인 시 별도 개발 |
| 11 | 플로팅 펫 상호작용 고도화 | 우클릭 메뉴 위치 개선 구현 완료 | 드래그/로밍 충돌 개선, 화면 밖 보정 |
