...
| 8 | 배포와 설치 | 자동 검증 완료, 로컬 `.dmg` 후보 생성/검증 완료, 더블클릭 설치/제거 command, 상태 확인 command, 설치본 freshness guard, checksum, release note draft 구성, unsigned draft release workflow와 public stable release gate 구성 | GitHub Actions 실제 실행, 깨끗한 사용자 환경의 더블클릭 설치/제거 검수, 공개 배포 signing/notarization |
...
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, XPC 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증, 장시간 덮개 닫힘 실사용 검증 완료, 더블클릭 helper 설치/제거 안내와 상태 확인 command 구성, 앱 내부 helper 설치/제거 버튼 1차 구현, helper 버튼 상태별 자동 검증, UI 검수 전 설치본 freshness guard와 수동 UI 검수 preflight 구성 | 앱 내부 helper 버튼 실제 클릭 검수, 공개 배포용 helper installer gate |
