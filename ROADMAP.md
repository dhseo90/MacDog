...
| 데스크톱 펫 보기 | 우클릭 메뉴, Milestone 6에서 활성화 | 해당 없음 | Codex Pup을 데스크톱 표시 영역에 표시 |
...
- 데스크톱 펫 표시 동작은 이 milestone에서 예약하고, 실제 표시 영역은 Milestone 6에서 활성화한다.
...
- 1차 구현은 고정 위치 플로팅으로 시작하고, 후속으로 화면 안에서 짧게 뛰어다니는 roaming motion을 추가한다.
