...
| 11 | Privileged Helper 설치 통합 | helper 우선 sleep 제어 코드 구현, helper-only 실제 설치/삭제/재설치, XPC 조회/변경/복구, 최신 UI 설정 왕복, Chrome Remote Desktop 상태의 약 10분 덮개 닫힘 실기 검증, 장시간 덮개 닫힘 실사용 검증 완료, 더블클릭 helper 설치/제거 안내와 상태 확인 command 구성, 앱 내부 helper 설치/제거 버튼 1차 구현, UI 검수 전 설치본 freshness guard 구성 | 앱 내부 helper 버튼 실제 클릭 검수, 공개 배포용 helper installer gate |
