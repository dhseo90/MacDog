...
25. live Codex app-server가 성공하면 5시간/주간 window가 모두 있는 success cache여야 하고, live fetch가 실패하면 error snapshot이어야 합니다.
26. 5시간/주간 window가 없는 `0% 사용 / 100% 남음` 형태의 success cache가 생성되면 릴리즈 종료 실패로 봅니다.
27. `./script/cleanup_release_smoke_state.sh --apply`로 release smoke 잔여물을 정리합니다.
28. `./script/verify_release_final_state.sh --version X.Y.Z`가 통과해야 release smoke 종료로 봅니다.
29. 릴리즈 publish와 final smoke가 끝난 뒤 로컬/원격 `<release-branch>`를 정리합니다.
30. 브랜치 정리 후 `git branch -a`에서 `<release-branch>`와 `origin/<release-branch>`가 없는지 확인합니다.
