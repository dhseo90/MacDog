...
사용자가 "골든리트리버 컨셉으로 이미지 생성"처럼 캐릭터 컨셉만 지시하면, 에이전트는 아래 기준의 전체 캐릭터 세트를 같은 캐릭터로 생성해야 한다.
일부 이미지만 새 컨셉으로 바꾸거나, 서로 다른 그림체/종/형태/소품을 섞어서는 안 된다.

#### 10.1.1 기준 이미지와 캐릭터 동일성

1. 기준 이미지는 menu bar runner 캐릭터다. 최초 승인된 menu bar runner의 얼굴형, 귀, 꼬리, 몸 비율, 색, 무늬, 선 두께, 표정 범위를 해당 캐릭터의 정체성으로 본다.
2. desktop pet, popover tab button, 설정 탭 캐릭터 미리보기는 모두 기준 이미지와 같은 캐릭터여야 한다.
3. 새 이미지가 기준 캐릭터와 다르게 보이면 실패로 본다. "비슷한 강아지", 로봇처럼 보이는 이미지, 다른 종처럼 보이는 이미지, 장난감/마스코트처럼 변형된 이미지는 사용할 수 없다.
4. 모든 이미지는 투명 배경 PNG여야 하며, 귀/꼬리/머리/발이 잘리지 않도록 충분한 여백을 둔다.
5. 작은 크기에서 읽히도록 단순한 실루엣과 명확한 색 대비를 유지한다. 과한 장식, 복잡한 배경, 텍스트 삽입은 금지한다.
6. 한 캐릭터 세트 안에서는 그림체, 광원, 윤곽선, 채도, 비율이 흔들리면 안 된다.

#### 10.1.2 Menu Bar Runner 생성 기준

1. menu bar runner는 캐릭터 세트의 기준 이미지다. 가장 먼저 생성하고 사용자 승인을 받아야 한다.
2. runner는 "달리는 모습"의 반복 애니메이션이어야 한다.
3. 기본 구성은 `Sources/MacDog/Resources/Runner/pup-runner-0.png`부터 `pup-runner-7.png`까지 8프레임이다.
4. 8프레임은 같은 캔버스, 같은 기준선, 같은 중심점으로 맞춘다. 애니메이션 중 캐릭터 크기나 위치가 튀면 실패로 본다.
5. 프레임은 옆을 향해 달리는 실루엣을 기본으로 한다. 머리, 귀, 몸통, 다리, 꼬리 움직임이 자연스럽게 순환해야 한다.
6. menu bar에서는 매우 작게 보이므로 디테일보다 캐릭터 식별성, 실루엣, 움직임의 리듬을 우선한다.
7. runner가 승인되기 전에는 desktop pet이나 tab button 최종 리소스를 확정하지 않는다.

#### 10.1.3 Desktop Pet 생성 기준

1. desktop pet은 승인된 menu bar runner와 같은 캐릭터여야 한다.
2. 기본 리소스 구조와 프레임 수는 현재 `MacDogCharacterProfile.codexPup.desktopPet` 계약을 따른다.
3. 이동 포즈는 다음 세트를 생성한다.
...
4. 왼쪽 이동은 별도 리소스를 만들지 않는 한 오른쪽 이동 프레임의 좌우 반전을 기준으로 한다. 별도 왼쪽 리소스를 추가하려면 프로필 구조와 테스트를 함께 갱신해야 한다.
5. 상태 포즈는 다음 세트를 생성한다.
...
6. 각 포즈 세트는 같은 캔버스, 같은 캐릭터 크기, 같은 기준선으로 정렬한다.
7. 데스크톱에서 걸어다닐 때 상하좌우 전환이 어색하지 않아야 하며, 방향별로 다른 캐릭터처럼 보여서는 안 된다.

#### 10.1.4 Popover Tab Button 생성 기준

1. tab button 이미지는 `Sources/MacDog/Resources/PopoverTabs`에 256x256 PNG로 생성한다.
2. tab button은 새 캐릭터를 상상해 그리지 않는다. 승인된 desktop pet 포즈 또는 동일 캐릭터로 검증된 원본에서 파생한다.
3. 기본 파일은 다음 5개다.
...
4. 각 tab button은 같은 캐릭터를 유지하되 탭 주제가 즉시 구분되어야 한다.
...
5. 배지는 작고 단순해야 하며 캐릭터 얼굴, 귀, 꼬리를 가리면 안 된다.
6. tab button에는 텍스트를 넣지 않는다. 64x64로 축소되어도 캐릭터와 탭 주제가 읽혀야 한다.
7. 각 버튼은 selected border 안에서 상단/하단이 잘리지 않아야 한다. 렌더 후 실제 popover에서 확인한다.
8. 모든 tab button은 `MacDogCharacterProfile`과 `codex-pup-tab-art.json` 또는 새 캐릭터의 동등한 manifest에 연결되어야 한다.

#### 10.1.5 설정 탭 적용 기준

1. 캐릭터를 변경하면 menu bar runner, desktop pet, popover tab button, 설정 탭 캐릭터 미리보기를 하나의 세트로 함께 적용한다.
2. 설정 탭 캐릭터 미리보기는 현재 선택된 캐릭터의 실제 이미지를 보여야 한다.
3. 설정 탭에서 선택된 캐릭터가 다른 리소스와 다르게 보이면 실패로 본다.
4. fallback 아이콘은 로딩 실패를 드러내는 용도로만 사용한다. fallback 아이콘을 정상 캐릭터처럼 보이게 두면 안 된다.
5. 캐릭터 변경 UI를 추가하거나 바꾸면 `MacDogCharacterProfile`, 관련 manifest, 검증 스크립트, 스크린샷 테스트를 함께 갱신한다.

#### 10.1.6 임시 이미지와 미사용 리소스 삭제 규칙

1. 이미지 후보 생성 중 만들어진 임시 파일은 저장소에 넣지 않는다.
2. 임시 이미지는 작업용 임시 디렉터리에만 둔다. README, ROADMAP, AGENTS, 테스트 fixture, 앱 리소스, 스크린샷 산출물에서 임시 이미지를 참조하지 않는다.
3. 사용자가 최종 이미지를 선택하면 선택된 파일만 정식 리소스 경로로 복사하거나 재생성한다.
4. 최종 선택이 끝난 즉시 모든 임시 생성 이미지를 삭제한다.
5. 삭제 후에는 임시 디렉터리, `Assets/Generated`, 문서 스크린샷 산출물, 앱 리소스 경로에 미사용 이미지가 남아 있지 않은지 확인한다.
6. 임시 이미지가 산출물 또는 앱 번들에 포함되어 삭제할 수 없으면, 현재 승인된 캐릭터 이미지로 교체한 뒤 기존 임시 이미지를 삭제한다.
7. 사용자에게 최종 보고할 때는 남아 있는 정식 리소스와 삭제한 임시 리소스를 구분해서 보고한다.

#### 10.1.7 필수 검증

캐릭터 이미지 세트를 생성하거나 교체한 뒤에는 최소한 아래를 실행한다.
...
가능하면 최신 앱을 실행해 menu bar runner, desktop pet, popover tab button, 설정 탭 미리보기를 직접 확인한다.
직접 확인하지 않았다면 `UI 확인 미수행`으로 보고한다.
