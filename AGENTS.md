...
16. 개발 작업에는 사용 가능한 경우 Superpowers(supers) 워크플로를 적용해 요구사항 파악, 계획, 테스트, 검증을 진행합니다.
17. 독립적인 조사, 구현, 검토가 병렬로 가능하고 사용자 지정 범위를 벗어나지 않는 경우에는 필요에 따라 서브 에이전트를 활용할 수 있습니다. 서브 에이전트 사용도 보안, 승인, 검증, 보고 규칙을 동일하게 따릅니다.
...
24. 설치된 CLI 또는 빌드된 CLI로 `./script/verify_usage_fetch_cache_contract.sh --cli <codex-usage-path>`를 실행합니다.
25. live Codex app-server가 성공하면 5시간/주간 window가 모두 있는 success cache여야 하고, adjacent `usage-weekly-history.json`에 최소 1개 sample이 생성되어야 합니다.
26. 성공 fetch의 stderr에는 `history append: stored ... recordingStartedAt=...` diagnostic이 있어야 하며, cache는 갱신됐는데 weekly history 또는 append diagnostic이 없으면 릴리즈 종료 실패로 봅니다.
27. live fetch가 실패하면 error snapshot이어야 합니다.
28. 5시간/주간 window가 없는 `0% 사용 / 100% 남음` 형태의 success cache가 생성되면 릴리즈 종료 실패로 봅니다.
29. `./script/cleanup_release_smoke_state.sh --apply`로 release smoke 잔여물을 정리합니다.
30. `./script/verify_release_final_state.sh --version X.Y.Z`가 통과해야 release smoke 종료로 봅니다.
31. 릴리즈 publish와 final smoke가 끝난 뒤 로컬/원격 `<release-branch>`를 정리합니다.
32. 브랜치 정리 후 `git branch -a`에서 `<release-branch>`와 `origin/<release-branch>`가 없는지 확인합니다.
...
11. 주간 잔여량 그래프는 같은 `resetsAt` window 안에서 표시 잔여율이 증가하지 않도록 그립니다.
12. OpenAI가 주간 한도를 실제 리셋해 `resetsAt`이 바뀐 경우에만 이전 history와 분리하고 새 타임라인을 왼쪽 100%에서 시작합니다.
